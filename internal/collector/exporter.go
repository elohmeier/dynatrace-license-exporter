...
func New(cfg config.Config, client LicenseClient, hostTargets []HostTarget, logger *slog.Logger) *Exporter {
	...
	d := descriptors{
		...
		hostInfo:            prometheus.NewDesc(namespace+"_license_host_info", "Billing and bounded Dynatrace metadata for one billed host.", labels("environment_id", "environment", "host_id", "host", "monitoring_mode", "host_unit_band", "host_group", "network_zone", "auto_injection"), nil),
	...
	for _, env := range snapshot.Environments {
		...
		if e.cfg.IncludeHosts {
			for _, host := range env.Hosts {
				...
				e.emit(ch, e.desc.hostInfo, prometheus.GaugeValue, 1,
					env.ID, env.Name, host.ID, host.Name, host.MonitoringMode, hostUnitBandLabel(host.EstimatedHostUnits),
					metadataLabel(host.Platform.HostGroup), metadataLabel(host.Platform.NetworkZone), metadataLabel(host.Platform.AutoInjection),
				)
...
func (e *Exporter) allDescriptors() []*prometheus.Desc {
	return []*prometheus.Desc{
		...
		e.desc.hostMemoryBytes, e.desc.hostInfo, e.desc.hostKubernetesInfo,
	}
}

func hostUnitBandLabel(value float64) string {
